"""
Borrow from timm(https://github.com/rwightman/pytorch-image-models) and T2T-ViT
"""
import torch
...
class NormalCell(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                ...
                shift_size=0, window_size=0, gamma=False, init_values=1e-4, img_size=224, combine=False):
        super().__init__()
    ...
    def forward(self, x, size):
        ...
        assert n == H*W
        origin = x
        shortcut = x.reshape(b, H, W, c).permute(0, 3, 1, 2).contiguous()
        if self.tokens_type == 'window':
            ...
            if self.combine:
                shortcut = nn.functional.pad(shortcut, (padding_left, padding_right, padding_top, padding_down))
        ...
        if self.class_token:
            ...
            convX = self.gamma2 * self.PCM(shortcut).permute(0, 2, 3, 1).contiguous()
            if self.tokens_type == 'window' and self.combine:
                convX = convX[:, padding_top:padding_top+H, padding_left:padding_left+W, :]
            convX = self.drop_path(convX.reshape(b, n, c))
            x = origin + self.drop_path(self.gamma1 * x) + convX
