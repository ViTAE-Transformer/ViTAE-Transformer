...
def find_images_and_targets(folder, types=IMG_EXTENSIONS, class_to_idx=None, leaf_name_only=True, sort=True, idxFilesRoot=''):
    labels = []
    filenames = []
    for root, subdirs, files in os.walk(folder, topdown=False):
        ...
        txtFiles = None
        if idxFilesRoot != '':
            txtFiles = open(os.path.join(idxFilesRoot, '{}.txt'.format(label)), 'r').readlines()
            txtFiles = [ele.strip() for ele in txtFiles]
        for f in files:
            base, ext = os.path.splitext(f)
            if ext.lower() in types:
                if txtFiles is not None and f in txtFiles:
                    filenames.append(os.path.join(root, f))
                    labels.append(label)
                elif txtFiles is None:
                    filenames.append(os.path.join(root, f))
                    labels.append(label)
